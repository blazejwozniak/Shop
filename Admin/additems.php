...
$item_price = $_POST['item_price'];
 ...
 $check_item="select * from items WHERE item_name='$item_name'";
    ...
    {
echo "<script>alert('Przedmiot już instieje!')</script>";
			...
			if(in_array($imgExt, $valid_extensions)){			
				...
				if($imgSize < 5000000)				{
					...
					mysqli_query($dbcon,$saveitem);
					 echo "<script>alert('Przedmiot pomyślnie zapisano!')</script>";
				 ...
				 echo "<script>alert('Tylko formaty JPG, JPEG, PNG & GIF są obsługiwane.')</script>";
					 echo "<script>window.open('items.php','_self')</script>";
